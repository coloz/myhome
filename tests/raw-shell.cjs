...
(async()=>{
 ...
 try{
  ...
  await require('./scheme-ui.cjs').selectScheme(page,'raw-shell');await ready();s=await snap();assert.equal(s.entities.length,1);assert(s.architecture.walls.find(n=>n.id===w.id),'Copy reset changed original design');
