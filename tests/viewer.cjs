...
(async()=>{
 ...
 await page.setViewportSize({width:390,height:844});await page.getByRole('button',{name:'方案与房间',exact:true}).click();await page.getByRole('button',{name:'查看客厅',exact:true}).click();await page.getByRole('button',{name:'户型图',exact:false}).first().click();await page.waitForTimeout(800);
