...
import {FLOOR_STYLES,WALL_COLORS,DEFAULT_SOLID_WALL_COLOR,normalizeWallColor,type FinishPreset} from './finish-presets';
...
export class FinishMaterials {
 ...
 private solids=new Map<string,T.MeshStandardMaterial>();
 ...
 solid(room:string,color=DEFAULT_SOLID_WALL_COLOR){
  // One reusable untextured material per room; dragging a picker creates no texture or material cache entries.
  const hex=normalizeWallColor(color)??DEFAULT_SOLID_WALL_COLOR;let material=this.solids.get(room);
  if(!material){material=new T.MeshStandardMaterial({roughness:.85});material.name='纯色墙面 · '+room;this.solids.set(room,material);}
  material.color.set(hex);material.userData={finishId:'solid',finishKind:'wall',room,wallColor:hex};return material;
 }
 ...
 debug(){return [...[...this.entries.values()].map(e=>({id:e.preset.id,kind:e.material.userData['finishKind'],ready:!!e.material.map,error:e.error,repeat:e.material.map?.repeat.toArray()})),...[...this.solids.entries()].map(([room,m])=>({id:'solid',kind:'wall',room,color:m.color.getHexString(),ready:true,error:false,textured:!!m.map}))];}
 destroy(){this.disposed=true;for(const t of this.textures)t.dispose();for(const e of this.entries.values())e.material.dispose();for(const m of this.solids.values())m.dispose();this.images.clear();this.entries.clear();this.solids.clear();}
}
