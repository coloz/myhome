...
import { FinishMaterials } from './finish-materials';
import { assignMetricWallUV } from './finish-uv';
import { GLTFLoader } from 'three/addons/loaders/GLTFLoader.js';
import { OrbitControls } from 'three/addons/controls/OrbitControls.js';
import { RoomEnvironment } from 'three/addons/environments/RoomEnvironment.js';
import { facadeCutawayGroup, nearestWallAlongView } from './wall-visibility';
...
export class HomeViewer {
  ...
  onFinishError=(_message:string)=>{};
  private finishLibrary=new FinishMaterials(message=>this.onFinishError(message));
  ...
  async load(progress:(n:number)=>void) {
    ...
    this.model.traverse(obj=>{
      ...
      // Metadata may be inherited from a GLTF parent when a wall has multiple materials.
      const cutaway=facadeCutawayGroup(data['cutaway']??'',data['sourceName']);
      obj.userData['cutaway']=cutaway;
      this.parts.push({mesh:obj,room:data['room'],rooms:Array.from(data['rooms']??[data['room']]),layer,cutaway,box:new THREE.Box3().setFromObject(obj),original:obj.material,wallId:data['wallId']});
      if(layer==='finish'&&this.project.walls){
        const pos=obj.geometry.getAttribute('position'),uv:number[]=[];
        for(let i=0;i<pos.count;i++){const v=new THREE.Vector3().fromBufferAttribute(pos,i).applyMatrix4(obj.matrixWorld);uv.push(v.x,v.z);}
        obj.geometry.setAttribute('uv',new THREE.Float32BufferAttribute(uv,2));
      }
    });
    // A floor-to-ceiling opening and its solid end returns share one editable
    // wall record, but must not disappear as one oversized cutaway group.
    for(const w of this.project.walls??[]){
      const windows=w.openings.filter(o=>o.kind==='window'&&o.bottom===0);if(!windows.length)continue;
      const length=Math.hypot(w.b[0]-w.a[0],w.b[1]-w.a[1]),ux=(w.b[0]-w.a[0])/length,uz=(w.b[1]-w.a[1])/length;
      for(const p of this.parts){
        if(p.wallId!==w.id||p.layer!=='wall'||p.box.max.y-p.box.min.y<w.height-.01)continue;
        const distances=[p.box.min.x,p.box.max.x].flatMap(x=>[p.box.min.z,p.box.max.z].map(z=>(x-w.a[0])*ux+(z-w.a[1])*uz));
        const start=Math.min(...distances),end=Math.max(...distances);
        if(windows.some(o=>start<o.end-.03&&end>o.start+.03))continue;
        p.cutaway=w.id+':solid:'+start.toFixed(3)+':'+end.toFixed(3);p.mesh.userData['cutaway']=p.cutaway;
      }
    }
  ...
  applyStates() {
    ...
    for(const p of this.parts){
      ...
      if(this.mode==='plan'&&['window','door','ceiling','facade'].includes(p.layer))visible=false;
      ...
      if(p.layer==='wall'){
        const switched=mats.map(m=>{
          ...
          return (rid==='garden'||!this.states[rid]?.decorated)?this.raw:this.project.walls?this.finishMaterial('wall',(this.finishes[rid]??DEFAULT_FINISH).wall,rid,this.finishes[rid]?.wallColor):m;
        });
  ...
  private finishMaterial(kind:'wall'|'floor',id:string,room='',color?:string){
    const m=kind==='wall'&&id==='solid'?this.finishLibrary.solid(room,color):this.finishLibrary.get(kind,id);if(kind==='wall')this.cutMaterials.add(m);return m;
  }
  retryFinishTextures(){this.finishLibrary.retry();}
  protected waitForFinishTextures(){
    const materials=new Set<THREE.Material>();
    for(const p of this.parts)if(!p.disabled&&p.rooms.some(id=>this.states[id]?.visible)&&(p.layer!=='finish'||this.states[p.room]?.decorated))for(const m of Array.isArray(p.mesh.material)?p.mesh.material:[p.mesh.material])materials.add(m);
    return this.finishLibrary.ready(materials);
  }
  ...
  private cullWalls() {
    ...
    for(const p of this.parts){
      ...
      if(this.mode==='plan'&&['window','door','ceiling','facade'].includes(p.layer))vis=false;
  ...
  debug(){return {finishTextures:this.finishLibrary.debug(),webgl2:this.renderer.getContext() instanceof WebGL2RenderingContext,mode:this.mode,selected:this.selected,hiddenWalls:this.hiddenWalls,showCeiling:this.showCeiling,walk:this.walk.debug(),exterior:{visible:this.exterior.visible,trees:this.exterior.children.filter(o=>o.userData['treeId']).length,groundHeight:this.ground?.position.y},direction:this.camera.getWorldDirection(new THREE.Vector3()).toArray(),states:this.states,parts:this.parts.map(p=>({room:p.room,layer:p.layer,cutaway:p.cutaway,wallId:p.wallId,curveGroup:p.mesh.userData['curveGroup'],sourceName:p.mesh.userData['sourceName'],visible:p.mesh.visible,invalidMaterialGroups:Array.isArray(p.mesh.material)&&p.mesh.geometry.groups.length===0,raw:(Array.isArray(p.mesh.material)?p.mesh.material:[p.mesh.material]).includes(this.raw)})),triangles:this.renderer.info.render.triangles,camera:this.camera.position.toArray(),target:this.controls.target.toArray()};}
  destroy(){
    this.disposed=true;this.finishLibrary.destroy();this.walk.destroy();cancelAnimationFrame(this.frame);this.resize.disconnect();this.controls.dispose();this.renderer.domElement.removeEventListener('keydown',this.keyHandler);
    const mats=new Set([...this.parts.flatMap(p=>Array.isArray(p.original)?p.original:[p.original]),...this.cutMaterials]);
