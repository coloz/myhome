...
import { DEFAULT_SOLID_WALL_COLOR, SOLID_WALL_SWATCHES, normalizeWallColor } from './finish-presets';
...
export class App implements OnDestroy {
 ...
 solidSwatches=SOLID_WALL_SWATCHES;defaultSolidColor=DEFAULT_SOLID_WALL_COLOR;wallColorError=signal('');
 ...
 changeFinish(key:'wall'|'floor',value:string){this.wallColorError.set('');this.viewer?.setRoomFinish(this.finishRoom(),key,value);this.sync();}
 solidWallColor():string|null{
  const ids=this.finishRoom()==='all'?this.project()?.rooms.filter(r=>!r.greeneryOnly).map(r=>r.id)??[]:[this.finishRoom()];
  const colors=new Set(ids.map(id=>this.viewer?.finishes[id]?.wallColor??DEFAULT_SOLID_WALL_COLOR));return colors.size===1?[...colors][0]:null;
 }
 setSolidColor(value:string,continuous=false){
  const color=normalizeWallColor(value);if(!color){this.wallColorError.set('请输入有效的 HEX 色值，例如 #B7CAD3。');return;}
  this.wallColorError.set('');this.viewer?.setRoomWallColor(this.finishRoom(),color,continuous);this.sync();
 }
 endSolidColor(){this.viewer?.endWallColorEdit();}
